...
# Obj 2: mHPSC treatment effect

We now want to focus on mHPSC treatment effect

## Proteins associated with PSA

Lets examine 25 + 6 patients with serial samples

```{r}
df_b_serial <- df_meta_f2 %>% 
  filter(pattern %in% c("B_Serial_PrePost", "B_Pre_to_C_Pre")) %>% 
  filter(txt_stat_ordered %in% c("Pre1", "Post1")) %>% 
  select(mrn, HCI_cID, txt_stat_ordered) 
dim(df_b_serial)
length(unique(df_b_serial$mrn))
```

In these samples, run the PSA values

```{r}
df_psa_b <- df_b_final %>% select(HCI_cID, psa) %>% mutate(group = "Serial B")
df_psa_c <- df_c_final %>% select(HCI_cID, psa) %>% mutate(group = "Serial B_C") 

df_tmp <- rbind(df_psa_b, df_psa_c)

df_psa <- df_b_serial %>% left_join(df_tmp) %>% 
  mutate(txt_stat_ordered = factor(txt_stat_ordered, levels=c("Pre1", "Post1")))
```
...
```{r}
# Create the plot
ggplot(df_psa, aes(x = txt_stat_ordered, y = psa, group = mrn)) +
 geom_point(aes(color = txt_stat_ordered)) +
 geom_line() +
  #facet_wrap(~group) +
 theme_bw() +
