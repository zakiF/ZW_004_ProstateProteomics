...
> Update June 25 2025. This section is depreciated. Because this section was running DE only on the Psomagen cohort. We now want to combine all dataset (bridging) then run DE. 
...
```{r}
meta_psom_tmp <- meta_psom %>% select(-sample_id) 

df_meta <- df_pre_post_ori %>%   
...
```{r}
pca_cor %>% filter(cohort == "C_D") %>% 
ggplot(., aes(x=PC1, y=PC2, colour=OS)) +
  geom_point(size=2.5) +
  ylab(p_pca[[1]]$labels$y) +
  xlab(p_pca[[1]]$labels$x) +
  theme_classic() +
...
Replot the PCA based on the filtered proteins

```{r}
#ss <- df_meta %>% filter(txt_stat %in% c("Pre", "Pre2"))
#dat1_final_tmp <- dat1_final %>% filter(SampleID %in% ss$SampleID)
# Quick PCA 
p_pca <-
  dat1_final %>% 
  filter(!str_detect(SampleID, 'CS')) %>% 
...
Try to see if there is associateion between PC1 and OS
...
exp <- dat1_final %>%
  ...
  as.data.frame() 
rownames(exp) <- exp$OlinkID
exp <- exp[,-1]

pc <- prcomp(t(exp), scale. = TRUE)

pc1_scores <- pc$x[, 1]  # First principal component

cor(pc1_scores, df_meta$OS, method = "pearson")

summary(lm(pc$x[,1] ~ df_meta$OS))
```
