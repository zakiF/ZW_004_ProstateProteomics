...
# Read the files
meta_clin <- readxl::read_excel(file.path(wd$manishData, "2022_11_04_LS_prot_clin_data_ID_copy.xlsx")) %>%
...
# p3 <- ggplot(pca_cor, aes(x=PCX, y=PCY, colour=mrn_lab, 
#                     label = mrn_lab2)) +
#   geom_point(size=2.5) +
#   ylab(p_pca[[1]]$labels$y) +
#   xlab(p_pca[[1]]$labels$x) +
...
p_no <- 
  ggplot(pca_cor_no, aes(x=PC1, y=PC2, colour=cohort_simple)) +
  geom_point(size=2.5) +
  ylab(p_pca[[1]]$labels$y) +
  xlab(p_pca[[1]]$labels$x) +
  theme_classic() +
...
```{r}
pal.pca <- pal.cohort_main[as.factor(pca_cor_no$cohort_simple)]

plot3d(pca_cor_no[,1:3], 
       size=5,
       col = pal.pca)
```
...
dat_tt <- 
  olink_ttest(df = dat_NPX_anno_all,
              variable = 'c_a_vs_b')
as.data.frame(dat_tt) %>% head() %>% select(Assay, p.value, Adjusted_pval, Threshold)

dat_tt_2 <- 
    olink_ttest_2(df = dat_NPX_anno_all,
                variable = 'c_a_vs_b')
as.data.frame(dat_tt_2) %>% head() %>% select(Assay, p.value, Adjusted_pval, Threshold)


# Plot example
dd <- dat_NPX_anno_all %>% filter(Assay == "CDH6")
...
Try to compare two-groups with limma - Decided not to use this as the adjusted p-value from limma is very high
...
cD <- dat_NPX_anno_all %>% select(SampleID, c_a_vs_b) %>% na.omit() %>% distinct() 
colnames(cD) <- c("SampleID", "group")

# Convert to wide format
cts <- dat_NPX_anno_all %>% filter(SampleID %in% cD$SampleID) %>% select(OlinkID, NPX, SampleID) %>% tidyr::pivot_wider(names_from = SampleID, values_from = NPX) %>% as.data.frame()
row.names(cts) <- cts$OlinkID
cts <- cts[,cD$SampleID]


# Do the DE
# https://support.bioconductor.org/p/9135581/
# https://www.biostars.org/p/496806/
design <- model.matrix(~0 + group, cD)
fit <- lmFit(cts, design=design)

# We extract the DE genes in each comparison 
# Y vs N
contrast.matrix <- makeContrasts(groupN-groupY, levels=design)
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2, robust = F, trend = F)
dge_g1 <- 
  topTable(fit2, n = Inf, adjust.method = "BH", coef=1) %>%
  tibble::rownames_to_column(var = "UniqGeneID") %>%
  dplyr::select(UniqGeneID, logFC, P.Value, adj.P.Val) %>%
  rename(OlinkID = UniqGeneID) %>%
  left_join(df_anno)


# Another way of doing the DE
fit <- lmFit(cts, design=design, method="robust", maxit=1000)
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)
dge_g1 <- 
  topTable(fit2, n = Inf, adjust.method = "BH", coef=1) %>%
...
sel_cat <- dat_NPX_anno_all %>% dplyr::select(c_a_vs_b:c_d_vs_c) %>% colnames()
...
df_cat_b <- df_group_de %>%
  filter(Cat %in% c("c_a_vs_b", "c_c_vs_b", "c_d_vs_b") & Adjusted_pval <= cut_fdr)

df_cat_c <- df_group_de %>%
  filter(Cat %in% c("c_a_vs_c", "c_b_vs_c", "c_d_vs_c") & Adjusted_pval <= cut_fdr)

df_cat_d <- df_group_de %>%
  filter(Cat %in% c("c_a_vs_d", "c_b_vs_d", "c_c_vs_d") & Adjusted_pval <= cut_fdr)
```
...
kk <- dat_NPX_anno2 %>%
  dplyr::select(pt_idx, cohort_simple, chaarted_volume, s1_to_death_or_last_fu_mo, date_diff) %>%
...
kk_or <- kk %>% dplyr::arrange(cohort_simple, desc(s1_to_death_or_last_fu_mo), chaarted_volume)
