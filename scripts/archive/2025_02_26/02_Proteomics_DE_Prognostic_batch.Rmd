...
1. Perform DE on the dataset individually
2. Perform DE integrated DE
...
pdf(file.path(wd$outCurr, "Enricment_dotPlot_mCRPC_overExpressed.pdf"), width = 8, height = 4)
...
For prognostic analysis, we want to use the whole data set. Meaning we combine discovery and validation data. 
...
dat_snp2 <- dat_snp2 %>% 
  mutate(
    #OS = as.numeric(difftime(death_date, collection_date, units = "days")),
  ...
  dplyr::select(mrn, Genotype, Genotype2, met_stat, met_date, OS, death, enroll_disease_state, diag_glsn) #%>% 
...
percentage_data <- data %>%
  ...
  mutate(Percentage = (n / sum(n)) * 100,
         #Label = paste0(Genotype, "\n", round(Percentage, 1), "% (n=", n, ")"))
         Label = paste0(Genotype, ":   ", round(Percentage, 1), "% (n=", n, ")"))

# Create the bar plot
ggplot(percentage_data, aes(x = "", y = Percentage, fill = Genotype)) +
  ...
  geom_text(aes(label = Label), 
            ...
            color = "white") +
  labs(
    title = paste("Genotype Distribution (Total n =", sum(percentage_data$n), ")"),
    x = "Genotype",
    y = "Percentage (%)"
  ) +
  theme_minimal() +
  ...
  scale_fill_manual(values = c("#7F7F7F", "firebrick3"), na.value = "black")
```
...
Do a survival analysis with gleasson
