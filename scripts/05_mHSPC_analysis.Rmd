...
if(exists("results_base_5yr") && exists("protein_hrs_5yr")) {
  ...
  # Combine univariate and metastatic-controlled results for all 10 proteins + PSA
  combined_forest_data <- results_base_5yr %>%
    ...
    # Add PSA univariate results
    bind_rows(psa_univariate_5yr %>% 
              select(OlinkID, Assay,
                     HR_univariate = HR,
                     Lower_univariate = Lower_Bound,
                     Upper_univariate = Upper_Bound,
    ...
    left_join(
      # Combine protein and PSA controlled results
      bind_rows(
  ...
  # Create ordering with log10(PSA) at the top, then proteins by descending HR
  protein_hr_order <- combined_forest_data %>%
    filter(Analysis == "Univariate", Assay != "log10(PSA)") %>%
    arrange(desc(HR)) %>%
    pull(Assay)
  
  protein_order <- c("log10(PSA)", protein_hr_order)
  ...
  ggsave(filename = file.path(wd$outCurr, "Forest_plot_univariate_vs_controlled.pdf"), 
         plot = final_combined_plot, width = 7, height = 5)
  ...
  cat("Shows all 10 proteins + log10(PSA) with both univariate and metastatic-controlled HRs\n")
}
```
