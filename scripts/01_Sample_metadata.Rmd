...
> As of June 2025, Manish's team, Enos and Becky provided an updated clinical file. So we will use this

```{r}
meta_file <- file.path(file.path(wd$d2024, "clinical_data/Proteomic Demographic 06-19-25.xlsx"))
```
...
Enos provided us with the HCI_id and updated sample collection date

```{r}
df_date <- read_xlsx(file.path(wd$d2024, "clinical_data/Demographics Patients_Proteomics 4-14-25 1.xlsx"),
sheet = 2)
dd <- df_date
df_date <- data.frame(
...
Lets also add the date of sample collection
...
Update May 29 2025. In the mHSPC cohort, we had sample with post and post2. Ie these samples 
...
Some samples have multiple collections. So it would be good to add the sample colelction date to the data. 
