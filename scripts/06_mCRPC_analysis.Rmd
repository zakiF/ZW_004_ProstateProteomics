...
write.csv(df_cpps_2, file.path(wd$outCurr, "CPPS_score_top10_proteins.csv"))
...
cpps_cox <- coxph(Surv(time_to_death, death) ~ CPPS_risk, data = df_cpps)

# View summary to get HR, CI, p-value
summary(cpps_cox)
...
cox_model_cpps_alone <- cph(Surv(time_to_death, death) ~ CPPS_risk, 
                           data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_combined <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp + CPPS_risk, 
                              data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
...
cox_model_clinical_nom <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp, 
                             data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_alone_nom <- cph(Surv(time_to_death, death) ~ CPPS_risk, 
                               data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
cox_model_cpps_combined_nom <- cph(Surv(time_to_death, death) ~ log10_psa + log10_alp + CPPS_risk, 
                                  data = df_cpps_complete, x = TRUE, y = TRUE, surv = TRUE)
